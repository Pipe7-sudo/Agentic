...
export default function UsagePage() {
  ...
  return (
    <div className="max-w-[900px] mx-auto flex flex-col gap-6">
      {/* Current Plan Banner */}
      <div className="relative bg-white dark:bg-[#080812] border border-slate-200 dark:border-white/[0.07] rounded-2xl p-6 shadow-sm dark:shadow-none overflow-hidden">
        <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-indigo-500/40 to-transparent" />
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="font-['Outfit'] text-[20px] font-bold text-slate-900 dark:text-white">Growth Plan</span>
          ...
          <a href="#" className="px-5 py-2.5 rounded-xl text-sm font-semibold text-white gradient-bg shadow-[0_4px_16px_rgba(99,102,241,0.25)] hover:-translate-y-px transition-all no-underline shrink-0">
      ...
      <div className="bg-white dark:bg-[#080812] border border-slate-200 dark:border-white/[0.07] rounded-2xl p-6 shadow-sm dark:shadow-none">
        <div className="flex items-end justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5"><span className="font-bold text-slate-800 dark:text-slate-200">{TOTAL_USED}</span> of {TOTAL_LIMIT} credits used ({pct}%)</p>
          </div>
        ...
        <div className="flex flex-col gap-3">
          {BARS.map(b => (
            <div key={b.label} className="flex items-center gap-3">
              <span className="text-[12px] text-slate-600 dark:text-slate-400 w-[140px] shrink-0">{b.label}</span>
              ...
              <span className="text-[12px] text-slate-500 dark:text-slate-500 w-[40px] text-right">{b.used}</span>
            </div>
          ))}
        </div>
      </div>
